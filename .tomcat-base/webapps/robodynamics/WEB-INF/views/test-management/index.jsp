...
<div class="rd-shell">
  <section class="rd-hero">
    <div>
      <p class="rd-eyebrow">Robo Dynamics Assessment Ops</p>
      <h1 class="rd-hero-title">Test Management</h1>
      <p class="rd-hero-sub">
        Manage test schedules, mappings, and learner coverage from a single workspace.
      </p>
    </div>
    <div class="rd-hero-meta">
      <div class="rd-meta-card">
        <div class="rd-meta-label">Role Access</div>
        <div class="rd-meta-value">Admin, Mentor, Parent</div>
      </div>
      <div class="rd-meta-card">
        <div class="rd-meta-label">Views</div>
        <div class="rd-meta-value">List and Calendar</div>
      </div>
    </div>
  </section>
...
<div class="container px-0">
  ...
  <div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <c:if test="${canCreate}">
      <a href="${pageContext.request.contextPath}/test-management/new" class="btn btn-primary">+ New Test</a>
    </c:if>
  </div>
      ...
      <table class="table table-sm table-striped align-middle">
        ...
        <c:forEach var="t" items="${tests}">
          <tr>
            ...
            <!-- Title (link to view) -->
            <td>
              <a href="${pageContext.request.contextPath}/test-management/view?testId=${t.testId}">
                <c:out value="${t.testTitle}"/>
              </a>
            </td>

            <!-- Course -->
            <td><c:out value="${t.course != null ? t.course.courseName : '-'}"/></td>

            <!-- Offerings -->
            <td>
              <c:choose>
                <c:when test="${not empty offeringsByTest and not empty offeringsByTest[t.testId]}">
                  <c:forEach var="o" items="${offeringsByTest[t.testId]}">
                    <span class="badge text-bg-light border"><c:out value="${o}"/></span>
                  </c:forEach>
                </c:when>
            ...
            <td>
              <c:choose>
                <c:when test="${not empty gradesByTest and not empty gradesByTest[t.testId]}">
                  <c:forEach var="g" items="${gradesByTest[t.testId]}">
                    <span class="badge text-bg-info-subtle border"><c:out value="${g}"/></span>
                  </c:forEach>
                </c:when>
                <c:otherwise>-</c:otherwise>
              </c:choose>
            ...
            <!-- Students (Parent) -->
            <td>
              <c:set var="rows" value="${enrollSummaries[t.testId]}"/>
              <c:set var="cnt"  value="${
              ...
              <c:if test="${not empty rows}">
                <c:forEach var="r" items="${rows}" varStatus="s">
                  <c:if test="${s.index < 3}">
                    <span class="badge text-bg-light border me-1 mb-1">
                      ...
                      <c:if test="${not empty r.parentName}">
                ...
                <c:if test="${rows.size() > 3}">
                  <span class="badge text-bg-secondary me-1 mb-1">+<c:out value="${rows.size() - 3}"/> more</span>
                </c:if>
              </c:if>

              <!-- total count -->
              <span class="badge text-bg-secondary me-2"><c:out value="${cnt}"/></span>
            ...
            <td>
              <div class="btn-group">
                ...
                <c:if test="${canDelete}">
                  <form action="${pageContext.request.contextPath}/test-management/delete"
...
<!-- Enrollments Modal (lazy loads per test) -->
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <input type="text" id="modalFilter" class="form-control form-control-sm" placeholder="Filter by student or parent..."/>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (enrollModal) {
    enrollModal.addEventListener('show.bs.modal', function (ev) {
      ...
      fetch(base + '?' + params.toString())
        ...
        .catch(() => { enrollContent.innerHTML = '<div class="text-danger">Failed to load enrollments.</div>'; });
    });
    ...
    modalFilter?.addEventListener('input', function(){
      ...
      const filtered = currentData.filter(d =>
        ...
        (d.offeringName|| '').toLowerCase().includes(q)
      );
  ...
  function renderEnrollments(data) {
    ...
    let html = '';
    byOffering.forEach((byGrade, offLabel) => {
      html += `<div class="mb-3">
                 <h6 class="mb-2">${escapeHtml(offLabel)}</h6>`;
      byGrade.forEach((students, grade) => {
        html += `<div class="mb-2">
                   <div class="fw-semibold">Grade: ${escapeHtml(grade)} <span class="badge text-bg-secondary ms-1">${students.length}</span></div>
                   <ul class="mb-0">`;
        students.forEach(s => {
          const stu = s.studentName || ('ID ' + (s.studentId ?? ''));
