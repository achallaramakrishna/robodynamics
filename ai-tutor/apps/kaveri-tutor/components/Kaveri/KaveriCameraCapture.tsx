...
interface Props {
  targetChar: string;
  studentName: string;
  kannadaLevel: "native" | "some" | "beginner" | "zero";
...
export default function KaveriCameraCapture({
  targetChar,
  studentName,
  kannadaLevel,
  onResult,
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const resp = await fetch("/api/kaveri/check-letter", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ imageBase64: base64, targetChar, studentName, kannadaLevel }),
      });
  ...
  }, [capturedDataUrl, targetChar, studentName, kannadaLevel, isEnglish, onResult, onSpeak]);
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 900, color: "#172033", marginTop: 3, lineHeight: 1.2 }}>
              {isEnglish
                ? `Show Kaveri your "${targetChar}"!`
                : `ಕಾವೇರಿಗೆ ನಿಮ್ಮ "${targetChar}" ತೋರಿಸಿ!`}
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <button onClick={submit} style={{
                ...
                ✓ {isEnglish ? "Check it!" : "ಕಾವೇರಿಗೆ ಕಳುಹಿಸಿ!"}
              </button>
            </div>
          </>
        )}

        {/* ── ANALYZING: Kaveri thinking ── */}
        {camState === "analyzing" && (
          <div style={{ textAlign: "center", padding: "28px 0" }}>
            <div style={{
            ...
            <div style={{ fontSize: 17, fontWeight: 900, color: "#7c3aed", marginBottom: 6 }}>
              {isEnglish ? "Kaveri is checking…" : "ಕಾವೇರಿ ನೋಡುತ್ತಿದ್ದಾರೆ…"}
            </div>
