...
export type LanguageMode = 'kannada-full' | 'kannada-english' | 'english-simplified';
...
const STORAGE_KEY = 'kaveri_language_preference';
...
export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [language, setLanguageState] = useState<LanguageMode>('kannada-full');
  ...
  useEffect(() => {
    const loadPreference = async () => {
        ...
        if (userId && userId > 0) {
          try {
            ...
            if (response.ok) {
              ...
              if (['kannada-full', 'kannada-english', 'english-simplified'].includes(pref)) {
      ...
      try {
        ...
        if (stored && ['kannada-full', 'kannada-english', 'english-simplified'].includes(stored)) {
...
export const useLanguage = (): LanguageContextType => {
  ...
  if (!context) {
    ...
    return { language: 'kannada-full', setLanguage: () => {}, isLoading: false };
  }
  return context;
};
