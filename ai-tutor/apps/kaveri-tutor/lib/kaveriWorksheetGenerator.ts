/**
 * Kaveri AI — Offline Practice Worksheet Generator
 *
 * Draws an A4 practice worksheet on an off-screen canvas (so Kannada
...
export interface WorksheetData {
  lessonId:    string;
  lessonTitle: string;   // e.g. "ಅ for ಅಕ್ಕ"
  char:        string;   // Kannada letter, e.g. "ಅ"
  wordKannada:   string;   // e.g. "ಅಕ್ಕ"
  wordEnglish: string;   // e.g. "Elder Sister"
...
export async function generateWorksheet(data: WorksheetData): Promise<void> {
  ...
  ctx.fillStyle = "white";
  ctx.fillText("🎙 KAVERI AI TUTOR", PAD, 52);
  ...
  ctx.fillText("Kannada Practice Sheet — Print · Trace · Upload · Earn XP! ⭐", PAD, 84);
  ...
  ctx.fillStyle = SOFT;
  ctx.fillText(`Sound: "${data.romanSound}" · Word: ${data.wordKannada} (${data.wordEnglish})`, PAD + 80, 188);
  ...
  ctx.fillStyle = INK;
  ctx.fillText(data.wordKannada, PAD + cardH + 80, y + cardH / 2 + 18);
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    `📸  Scan & Upload at robodynamics.in/kaveri  →  Earn +50 Bonus XP! 🚀`,
    W / 2, footerY + 117,
  );
  ctx.textAlign = "left";
  ...
  ctx.fillStyle = "#cbd5e1";
  ctx.fillText(`Lesson: ${data.lessonId}  ·  kaveri.robodynamics.in`, PAD, footerY + 154);
  ctx.textAlign = "right";
  ctx.fillText("© RoboDynamics — Kaveri AI Tutor", W - PAD, footerY + 154);
  ctx.textAlign = "left";
  ...
  const filename = `kaveri-practice-${data.lessonId}-${data.char}.pdf`;
  pdf.save(filename);
}
