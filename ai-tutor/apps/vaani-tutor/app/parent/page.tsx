...
import { getProfileStats } from "@/lib/vaaniGamification";
...
const HINDI_LEVELS: { value: HindiLevel; label: string; desc: string; emoji: string }[] = [
  { value: "native", label: "Hindi at home", desc: "We speak Hindi daily", emoji: "🏠" },
  { value: "some",   label: "Some Hindi",    desc: "TV, school, friends",  emoji: "📺" },
  { value: "beginner", label: "Just starting", desc: "A few words only",  emoji: "🌱" },
  { value: "zero",   label: "Zero Hindi",    desc: "South Indian / no exposure", emoji: "⭕" },
];

export default function VaaniParentDashboard() {
  const [xp, setXp] = useState(0);
  const [streak, setStreak] = useState(0);
  const [masteredLetters, setMasteredLetters] = useState<string[]>([]);
  const [childName, setChildName] = useState("");
  const [childNameInput, setChildNameInput] = useState("");
  const [hindiLevel, setHindiLevel] = useState<HindiLevel | "">("");
  ...
  useEffect(() => {
    ...
    const loadParentView = async () => {
      ...
      setXp(profileStats.totalXP);
      setStreak(profileStats.currentStreak);
      setMasteredLetters(
        LETTER_SEQUENCE.slice(0, Math.min(LETTER_SEQUENCE.length, Math.floor(profileStats.totalXP / 120)))
      );

      const savedName = localStorage.getItem("vaani_student_name") || localStorage.getItem("childName") || "";
      const savedLevel = (localStorage.getItem("vaani_hindi_level") || "") as HindiLevel | "";
      setChildName(savedName);
      setChildNameInput(savedName);
      setHindiLevel(savedLevel);
        ...
        >
          <div>
            ...
            <h1 style={{ fontSize: 34, fontWeight: 900, color: "#172554", margin: 0 }}>
              Track your child&apos;s Hindi reading journey
            </h1>
            <p style={{ color: "#475569", margin: "10px 0 0", maxWidth: 700, lineHeight: 1.6 }}>
              Vaani helps families see confidence building step by step, from first sounds to letter mastery.
            </p>
          </div>
          <Link
            href="/"
        ...
        <section style={{
            ...
            <div>
              ...
              {!profileSaved && (
                <p style={{ color: "#64748b", margin: "6px 0 0", fontSize: 14 }}>
                  Vaani adapts her Hindi coaching style based on your child's background.
                </p>
              )}
            </div>
            {profileSaved && (
          ...
          {!profileSaved ? (
            <div style={{ display: "grid", gap: 20 }}>
              ...
              {/* Hindi level selector */}
              <div>
                <label style={{ fontSize: 14, fontWeight: 800, color: "#0f172a", display: "block", marginBottom: 12 }}>
                  How much Hindi does your child already know?
                </label>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
                  {HINDI_LEVELS.map(lv => (
                    <button
                      key={lv.value}
                      onClick={() => setHindiLevel(lv.value)}
                      style={{
                        border: hindiLevel === lv.value ? "2px solid #f97316" : "1px solid #e2e8f0",
                        background: hindiLevel === lv.value ? "#fff7ed" : "white",
                        borderRadius: 20, padding: "16px 14px", cursor: "pointer", textAlign: "left",
                        boxShadow: hindiLevel === lv.value ? "0 4px 14px rgba(249,115,22,0.18)" : "none",
              ...
              <button
                disabled={!childNameInput.trim() || !hindiLevel}
                onClick={saveProfile}
                style={{
                  border: "none",
                  cursor: (childNameInput.trim() && hindiLevel) ? "pointer" : "not-allowed",
                  background: (childNameInput.trim() && hindiLevel)
                    ? "linear-gradient(135deg, #f97316, #ef4444)"
                    : "rgba(23,32,51,0.10)",
                  color: (childNameInput.trim() && hindiLevel) ? "white" : "#94a3b8",
                  padding: "16px 22px", borderRadius: 18, fontWeight: 900, fontSize: 17,
                  transition: "all 0.2s ease",
                  boxShadow: (childNameInput.trim() && hindiLevel) ? "0 12px 28px rgba(249,115,22,0.28)" : "none",
                }}
              >
                Save child profile →
              </button>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              <div style={{ background: "#fff7ed", borderRadius: 20, padding: "16px 20px", border: "1px solid #fed7aa" }}>
                <div style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: 1.1, color: "#c2410c" }}>Hindi Level</div>
                <div style={{ fontSize: 18, fontWeight: 900, color: "#0f172a", marginTop: 6 }}>
                  {HINDI_LEVELS.find(l => l.value === hindiLevel)?.emoji}{" "}
                  {HINDI_LEVELS.find(l => l.value === hindiLevel)?.label}
                </div>
                <div style={{ fontSize: 12, color: "#64748b", marginTop: 3 }}>
                  {HINDI_LEVELS.find(l => l.value === hindiLevel)?.desc}
                </div>
              </div>
            </div>
          )}
