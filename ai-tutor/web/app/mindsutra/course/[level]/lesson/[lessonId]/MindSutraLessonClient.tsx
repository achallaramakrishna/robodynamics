...
function compactFractionSpacing(value: string) {
  return value.replace(/\s*\/\s*/g, "/").trim();
}

function parseComparableNumber(value: string) {
  const compact = compactFractionSpacing(value).replace(/,/g, "");
...
function answersMatch(userInput: string, expectedInput: string) {
  const normalizedUser = compactFractionSpacing(userInput);
  const normalizedExpected = compactFractionSpacing(expectedInput);
  ...
  const userNumber = parseComparableNumber(normalizedUser);
  const expectedNumber = parseComparableNumber(normalizedExpected);

  if (userNumber !== null && expectedNumber !== null) {
    return Math.abs(userNumber - expectedNumber) < 1e-9;
  }
...
function BoardPanel({
  ...
  const assetPath = rawAssetPath
    ? rawAssetPath.includes("?")
      ? rawAssetPath
      : `${rawAssetPath}?v=${LESSON_ASSET_VERSION}`
    : null;
...
export default function MindSutraLessonClient({ payload }: { payload: MindSutraLessonPayload }) {
  ...
  const activePracticeOptions = activePracticeVariant?.options ?? currentStep.practice?.options;
  ...
  function handleCheck() {
    ...
    // ── Standard Mode Logic ────────────────────────────────────────────────
    const normalizedExpected = compactFractionSpacing(String(numericAnswer));
    const isCorrect = answersMatch(answer, String(numericAnswer));
  ...
  function handleQuizQuestionCheck(index: number) {
    ...
    const normalizedExpected = compactFractionSpacing(String(q.answer));
    const isCorrect = answersMatch(quizAnswers[index] ?? "", String(q.answer));
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#F8FAFC", color: "#0F172A", fontFamily: "Segoe UI, sans-serif" }}>
      ...
      <header style={{ background: "#0F172A", color: "#F8FAFC", padding: isPhone ? "14px 16px" : "16px 24px", borderBottom: "1px solid #1E293B" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 12, color: "#94A3B8", textTransform: "uppercase", letterSpacing: 1.1 }}>{payload.course.title}</div>
            <div style={{ fontSize: isPhone ? 22 : 24, fontWeight: 800 }}>{payload.lesson.title}</div>
            <div style={{ fontSize: 14, color: "#CBD5E1" }}>{payload.lesson.sutra} · Step {stepProgress}</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <button
              ...
              style={{
                ...
                background: voiceEnabled ? "#1D4ED8" : "#0F172A",
                color: "#F8FAFC",
                borderRadius: 12,
                padding: "10px 14px",
                cursor: "pointer",
            ...
            <Link href={`/mindsutra/course/${payload.course.levelSlug}`} style={{ color: "#F8FAFC", textDecoration: "none", fontWeight: 700 }}>Back to Course</Link>
          </div>
        </div>
      </header>
                  ...
                  <>
                    {currentStep.practice.mode === "quiz" && currentStep.practice.questions ? (
                      ...
                      <>
                        ...
                        {currentStep.practice.mode === "mcq" ? (
                          <>
                            <div style={{ display: "grid", gap: 10, marginBottom: 12 }}>
                              {(activePracticeOptions ?? []).map((option) => {
                                const selected = answer === String(option);
                                return (
                                  <button
                                    key={option}
