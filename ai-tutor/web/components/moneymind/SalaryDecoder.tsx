...
export default function SalaryDecoder() {
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const earnings = SALARY_DATA.filter(i => i.type === "EARNING");
  const deductions = SALARY_DATA.filter(i => i.type === "DEDUCTION");
  ...
  const selectedItem = SALARY_DATA.find(i => i.id === selectedId);

  return (
    <div className="salary-decoder-container">
      <div className="decoder-header">
        <div className="badge">L6 LAB</div>
        <h3>Interactive Salary Decoder</h3>
      </div>

      <div className="payslip">
        <div className="payslip-top">
          <div className="company-logo">ROBO DYNAMICS PVT LTD</div>
          <p className="payslip-sub">Monthly Payslip: April 2026</p>
        </div>
      ...
      <div className="decoder-insight">
        {!selectedId ? (
          <div className="prompt-box">
            ...
            Click on any part of the salary slip to decode its secret!
          </div>
        ) : (
