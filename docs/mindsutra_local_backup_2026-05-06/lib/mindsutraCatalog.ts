...
export function resolvePlacementLevel(schoolGrade: number, quizScore: number): string {
  if (schoolGrade <= 4) return "L1"; // Grade 4 always starts at L1 — no quiz
  // quiz_score is out of 5
  if (quizScore >= 5) return "L4";
  if (quizScore === 4) return "L3";
  if (quizScore === 3) return "L2";
  return "L1";
}
