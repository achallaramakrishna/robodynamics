...
export default function MindSutraCourseClient({ payload }: { payload: MindSutraCoursePayload }) {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#F8FAFC", color: "#0F172A", fontFamily: "Segoe UI, sans-serif" }}>
      <header style={{ background: "#0F172A", color: "#F8FAFC", padding: isPhone ? "16px 16px" : "18px 24px", borderBottom: "1px solid #1E293B" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 12, color: "#94A3B8", textTransform: "uppercase", letterSpacing: 1.2 }}>MindSutra Course</div>
            <div style={{ fontSize: 24, fontWeight: 800 }}>{payload.course.title}</div>
            <div style={{ fontSize: isPhone ? 13 : 14, color: "#CBD5E1", lineHeight: 1.5 }}>{payload.course.subtitle} · {payload.course.tagline}</div>
          </div>
          <Link href="/mindsutra" style={{ color: "#F8FAFC", textDecoration: "none", fontWeight: 700 }}>Back to Levels</Link>
        </div>
      </header>
            ...
            <div style={{ background: "#FFFFFF", borderRadius: 18, border: "1px solid #E2E8F0", padding: isPhone ? 18 : 24 }}>
              ...
              <div style={{ borderRadius: 16, background: "#F8FAFC", border: "1px solid #E2E8F0", minHeight: isPhone ? 220 : 320, display: "grid", placeItems: "center", overflow: "hidden", padding: isPhone ? 12 : 0 }}>
                {typeof selectedPreview.boardPreview.data.assetPath === "string" ? (
                  <img
                    src={selectedPreview.boardPreview.data.assetPath}
