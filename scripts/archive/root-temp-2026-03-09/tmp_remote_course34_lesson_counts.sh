...
SELECT cs.course_session_id, cs.session_title,
       ...
       SUM(CASE WHEN qq.question_type IS NULL OR TRIM(qq.question_type)='' OR qq.question_type NOT IN ('multiple_choice','short_answer','long_answer','fill_in_blank') THEN 1 ELSE 0 END) AS other_type,
