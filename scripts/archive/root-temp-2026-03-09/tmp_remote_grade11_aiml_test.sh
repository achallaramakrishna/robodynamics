python3 - <<'PY'
import json, html, re, urllib.request, urllib.parse, http.cookiejar, urllib.error
...
TECH = ['AI','ML','MACHINE LEARNING','ARTIFICIAL INTELLIGENCE','COMPUTER','CODING','PROGRAMMING','SOFTWARE','ALGORITHM','DATA','PYTHON','ROBOT','AUTOMATION','ENGINEER','LOGIC','MATHEMAT']
NEG  = ['DOCTOR','MEDICAL','NURSE','BIOLOGY','PATIENT','SPORT','ATHLETE','FITNESS','COACH','PSYCHOLOG','LAW','LEGAL','FINANCE','ACCOUNT']
POS  = ['STRONGLY AGREE','ALWAYS','VERY HIGH','HIGH','EXCELLENT','DEFINITELY','YES','CONFIDENT','ENJOY','LOVE','FAVORITE']
...
def score_option(opt):
    ...
    for k in LOW:
        if k in t: s -= 4.0
    if any(x in t for x in ['MATH','PHYSICS','PROBLEM SOLVING','SYSTEMS']):
        s += 5.0
    return s

def best_code(opts):
    if not opts: return 'A'
    ranked=sorted(opts,key=score_option, reverse=True)
...
def rank_codes(opts):
    if not opts:
        return 'A>B>C>D'
    ranked=sorted(opts,key=score_option, reverse=True)
    return '>'.join([o['code'] for o in ranked])
...
'S_GOAL_01':'I want to become an AI/ML Developer and build intelligent products.',
'S_LIFE_01':'I enjoy solving real-world problems using coding, math, and AI systems.',
'S_HOBBY_01':'Coding, robotics, Kaggle-style data challenges, and algorithm puzzles',
'S_DISLIKE_01':'I do not prefer biology-heavy or purely non-technical career tracks',
'S_STYLE_01':'PRACTICAL',
'S_ACHIEVE_01':'Built Python mini-projects and solved logic problems consistently',
'S_FEAR_01':'Missing top engineering opportunities due to weak preparation',
'S_SUPPORT_01':'Need JEE/AI roadmap, weekly coding goals, and mentor feedback'
}
req('/aptipath/student/intake',payload)

# restart to ensure fresh attempt
...
'embed':'0','company':'',
'THINK_STORY_SUMMARY':'My strongest direction is AI/ML through consistent coding and math practice.',
'THINK_STORY_DECISION':'I will prioritize AI/ML engineering pathways over non-tech options.',
'THINK_STORY_ACTION':'I will execute a 90-day plan with Python, DSA, and ML mini-project milestones.'
}
q_order=[]
for q in questions:
    qc=str(q.get('questionCode') or '').strip()
    if not qc:
        continue
    q_order.append(qc)
...
if '/aptipath/student/report-intake' in result_loc:
    ...
    rp={
        'sessionId':rsid,
        'RFQ_01':'AI ML Developer is my strongest and most consistent direction now.',
        'RFQ_02':'Main constraint is time management between school and coding practice.',
        'RFQ_03':'In 30 days I will complete Python + one ML mini-project and weekly mocks.'
    }
...
titles=[t.strip() for t in re.findall(r'<div class="title">\s*([^<]+?)\s*</div>', html_result)]
print('SESSION_ID', sid)
print('QUESTION_COUNT', len(q_order))
print('RESULT_STATUS', code)
print('TOP10', ' | '.join(titles[:10]))
print('HAS_AI_ML', any('AI/ML' in t or 'AI Engineer' in t or 'Machine Learning' in t or 'Data Scientist' in t for t in titles[:10]))
PY
