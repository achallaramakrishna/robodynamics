...
DESCRIBE rd_course_offerings;

SELECT 'ENROLL', e.enrollment_id, e.student_id, co.course_id, e.status
FROM rd_student_enrollments e
JOIN rd_course_offerings co ON co.course_offering_id=e.course_offering_id
JOIN rd_users u ON u.user_id=e.student_id
WHERE LOWER(u.user_name)='krishvi' AND co.course_id=34;
