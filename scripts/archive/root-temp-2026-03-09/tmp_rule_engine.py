...
class VedicRuleEngine:
    ...
    CHAPTER_CATALOG: Dict[str, Dict[str, Any]] = {
        "L1_COMPLETING_WHOLE": {
            ...
            "estimatedMinutes": 28,
            "subtopics": [
                "Introduction to Completing the Whole",
                "Pairs That Make 10",
                "Ten Point Circle",
                "Completing the Next Ten",
                "Deficiency from Ten",
                "Mental Addition with Carry",
                "Grouping Numbers into Wholes",
                "Add Near a Base",
                "Subtract Near a Base",
            ],
            "learningGoals": [
                "Understand what completing the whole means in Vedic mental arithmetic.",
                "Use base-10 completion for faster addition and subtraction.",
                "Solve near-base arithmetic using clear, explainable steps.",
            ],
        },
        "L2_DOUBLING_HALVING": {
    ...
    CHAPTER_EXERCISE_FLOW: Dict[str, Dict[str, str]] = {
        "L1_COMPLETING_WHOLE": {
            "A": "Introduction to Completing the Whole",
            "B": "Pairs That Make 10",
            "C": "Ten Point Circle",
            "D": "Completing the Next Ten",
            "E": "Deficiency from Ten",
            "F": "Mental Addition with Carry",
            "G": "Grouping Numbers into Wholes",
            "H": "Add Near a Base",
            "I": "Subtract Near a Base",
        }
    }

    # â”€â”€ Grade scaling â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    @staticmethod
    ...
    # â”€â”€ L1 question type selectors (manual Practice A-O) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _q_completing_whole(self, group: str, scale: int = 2) -> Dict[str, Any]:
        ...
        if qtype_idx == 0:
            q = self._base_q(group, subtopic, "concept_intro")
            q.update({
        ...
        if qtype_idx == 1:
            ...
            ans = 10 - n
            q = self._base_q(group, subtopic, "pairs_make_10")
            q.update({
                "questionText": f"Exercise {group} ({subtopic}): Which number pairs with {n} to make 10?",
                "hint": f"Think: {n} + ? = 10.",
                "solution": f"{n} needs {ans} to complete 10.",
        ...
        if qtype_idx == 2:
            ...
            ans = start + jump
            q = self._base_q(group, subtopic, "circle_jump")
            q.update({
        ...
        if qtype_idx == 3:
            a = self._rng.choice([29, 39, 49, 59, 69])
            b = self._rng.randint(2, 8)
            ans = a + b
            jump = 10 - (a % 10)
            rem = b - jump
            q = self._base_q(group, subtopic, "complete_next_ten")
            q.update({
                "questionText": f"Exercise {group} ({subtopic}): {a} + {b} = ?",
                "hint": f"Reach {a + jump} first, then add the remainder.",
                "solution": f"{a}+{jump}={a+jump}, then +{rem} gives {ans}.",
                "expectedAnswer": str(ans),
            })
            return q
        ...
        if qtype_idx == 4:
            n = self._rng.randint(1, 9)
            ans = 10 - n
            q = self._base_q(group, subtopic, "deficiency_from_ten")
            q.update({
                "questionText": f"Exercise {group} ({subtopic}): What is the deficiency of {n} from 10?",
                "hint": f"{n} + ? = 10.",
                "solution": f"Deficiency of {n} from 10 is {ans}.",
                "expectedAnswer": str(ans),
            })
            return q

        if qtype_idx == 5:
            a = self._rng.randint(42 + scale * 3, 68 + scale * 4)
            b = self._rng.randint(24 + scale * 2, 39 + scale * 3)
            ans = a + b
            q = self._base_q(group, subtopic, "mental_addition_carry")
            q.update({
                "questionText": f"Exercise {group} ({subtopic}): {a} + {b} = ?",
                "hint": "Add tens, then units, then carry.",
                "solution": f"({a//10*10}+{b//10*10}) + ({a%10}+{b%10}) = {ans}.",
                "expectedAnswer": str(ans),
            })
            return q

        if qtype_idx == 6:
            a = self._rng.randint(11, 19)
            c = 20 - a
            b = self._rng.randint(6, 12)
            ans = a + b + c
            q = self._base_q(group, subtopic, "group_to_whole")
            q.update({
                "questionText": f"Exercise {group} ({subtopic}): {a} + {b} + {c} = ?",
                "hint": f"Group {a}+{c} first.",
                "solution": f"({a}+{c}) + {b} = 20 + {b} = {ans}.",
                "expectedAnswer": str(ans),
            })
            return q

        if qtype_idx == 7:
            near_tens = self._rng.choice([9, 19, 29, 39])
            a = self._rng.randint(24 + scale * 4, 72 + scale * 6)
            ans = a + near_tens
            nearby = (near_tens // 10 + 1) * 10
            deficit = nearby - near_tens
            q = self._base_q(group, subtopic, "add_near_base")
            q.update({
                "questionText": f"Exercise {group} ({subtopic}): {a} + {near_tens} = ?",
                "hint": f"Do +{nearby} then -{deficit}.",
                "solution": f"{a}+{nearby}={a+nearby}, then -{deficit} = {ans}.",
                "expectedAnswer": str(ans),
            })
            return q

        near_tens = self._rng.choice([8, 18, 28, 38, 48])
        a = self._rng.randint(max(near_tens + 12, 40), 96)
        ans = a - near_tens
        nearby = (near_tens // 10 + 1) * 10
        deficit = nearby - near_tens
        q = self._base_q(group, subtopic, "subtract_near_base")
        q.update({
            "questionText": f"Exercise {group} ({subtopic}): {a} - {near_tens} = ?",
            "hint": f"Do -{nearby} then +{deficit}.",
            "solution": f"{a}-{nearby}={a-nearby}, then +{deficit} = {ans}.",
            ...
            "visual": self._svg(
                ...
                f"<text x='20' y='44' font-size='18' fill='#1e293b'>{a} - {near_tens}</text>"
                f"<text x='20' y='70' font-size='13' fill='#64748b'>= {a} - {nearby} + {deficit} = {ans}</text>",
            ),
        })
        return q
