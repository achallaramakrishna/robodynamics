...
public class RDCalendarController {
    ...
    // If DB DATE is stored as midnight UTC, keep true to avoid previous-day shift in IST
    private static final boolean USE_UTC_FOR_DATE_COLUMNS = true;

    private static final DateTimeFormatter ISO_LOCAL_SECOND =
    ...
    public List<CalendarEventDTO> adminEvents(
        ...
        ZonedDateTime startZ = parseCalParam(startStr);
        ZonedDateTime endExclZ = parseCalParam(endStr); // FC end is exclusive

        LocalDate from = startZ.toLocalDate();
        LocalDate toIncl = endExclZ.minusNanos(1).toLocalDate();
    ...
    public List<CalendarEventDTO> mentorEvents(
        ...
        ZonedDateTime startZ = parseCalParam(startStr);
        ZonedDateTime endExclZ = parseCalParam(endStr);
    ...
    private List<CalendarEventDTO> buildEventsByOffering(Integer mentorId, LocalDate since, LocalDate toIncl) {
        ...
        for (RDCourseOffering off : offerings) {
            ...
            if (activeDays.isEmpty()) activeDays = EnumSet.allOf(DayOfWeek.class);
            ...
            for (LocalDate d = effFrom; !d.isAfter(effTo); d = d.plusDays(1)) {
                ...
                LocalDateTime sdt = LocalDateTime.of(d, startTime);
                LocalDateTime edt = LocalDateTime.of(d, endTime);
                if (!edt.isAfter(sdt)) edt = sdt.plusMinutes(60);

                Map<String, Object> ext = new LinkedHashMap<>();
                ext.put("offeringId", off.getCourseOfferingId());
                ext.put("offeringName", off.getCourseOfferingName());
                if (off.getCourse() != null) {
                    ext.put("courseId", off.getCourse().getCourseId());
                    ext.put("courseName", off.getCourse().getCourseName());
                }
                if (off.getInstructor() != null) {
                    ext.put("mentorId", off.getInstructor().getUserID());
                    String fn = nz(off.getInstructor().getFirstName());
                    String ln = nz(off.getInstructor().getLastName());
                    ext.put("mentorName", (fn + " " + ln).trim());
                }
                ext.put("studentsCount", enrolled);
                // ext.put("location", ...); ext.put("sessionType", ...); ext.put("notes", ...);

                CalendarEventDTO dto = new CalendarEventDTO()
                        .setId("O-" + off.getCourseOfferingId() + "-" + d)
                        .setTitle(off.getCourseOfferingName())
                        .setStart(sdt.format(ISO_LOCAL_SECOND))
                        .setEnd(edt.format(ISO_LOCAL_SECOND))
                        .setExtendedProps(ext);
    ...
    private static LocalTime safeStartTime(LocalTime t) { return (t != null) ? t : LocalTime.of(9, 0); }
    ...
    private ZonedDateTime parseCalParam(String raw) {
        try { return OffsetDateTime.parse(raw).toZonedDateTime(); } catch (Exception ignore) {}
        try { return ZonedDateTime.parse(raw); } catch (Exception ignore) {}
        try { return LocalDateTime.parse(raw).atZone(ZoneId.systemDefault()); } catch (Exception ignore) {}
        try { return LocalDate.parse(raw).atStartOfDay(ZoneId.systemDefault()); } catch (Exception ignore) {}
