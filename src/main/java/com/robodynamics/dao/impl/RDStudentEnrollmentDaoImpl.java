...
public class RDStudentEnrollmentDaoImpl implements RDStudentEnrollmentDao {
	...
	public List<RDEnrollmentReportDTO> getEnrollmentsByParentId(Integer parentId) {
	    ...
	    final String hql =
	        ...
	        "         case when s.lastName is null or s.lastName = '' then '' else concat(' ', s.lastName) end), " + // 1 studentName
	        "  s.grade, " + // 2
	        "  case when mom.userID = :parentId " +
	        "       then concat(coalesce(mom.firstName, ''), " +
	    ...
	    for (Object[] r : rows) {
	        ...
	        java.time.LocalDate enrollmentDate = (d == null)
	                ? null
	                : d.toInstant().atZone(java.time.ZoneId.systemDefault()).toLocalDate();

	        RDEnrollmentReportDTO dto = new RDEnrollmentReportDTO(
	                enrollmentId == null ? 0 : enrollmentId,
	                studentName,
	                grade,
	                parentName,
	                parentContact,
	...
	public List<RDStudentInfoDTO> getEnrolledStudentInfosByOfferingIdAndParent(Integer offeringId, Integer parentId) {
	    ...
	    List<RDStudentInfoDTO> results = session.createQuery(hql)
    ...
    public List<RDStudentInfoDTO> getChildrenByParentId(Integer parentId) {
        ...
        return factory.getCurrentSession()
                .createQuery(hql, RDStudentInfoDTO.class)
                .setParameter("pid", parentId)
                .getResultList();
    }
