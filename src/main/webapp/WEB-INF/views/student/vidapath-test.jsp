...
<body>
      ...
      <div class="panel">
        ...
        <div class="controls">
          ...
          <div class="grade-picker">
            <label for="gradeBandSelect">Student Grade Band</label>
            <select id="gradeBandSelect">
              <option value="GRADE_8">Grade 8</option>
              <option value="GRADE_9">Grade 9</option>
              <option value="GRADE_10" selected>Grade 10</option>
  ...
  <script>
    ...
    async function startSession() {
      const payload = {
        studentId,
        gradeBand: gradeBandSelect ? gradeBandSelect.value : 'GRADE_10'
      };
    ...
    function populateQuestions(questions) {
      ...
      if (!questions || questions.length === 0) {
        ...
        doneCard.className = 'question-card';
        doneCard.innerHTML = '<p>You have completed this grade-level question set.</p>';
        document.getElementById('questDeck').appendChild(doneCard);
        return;
      }
      ...
      questions.forEach(question => {
        ...
        const parsedMcq = parseMcqQuestion(question.questionText);
        text.textContent = parsedMcq.prompt;
        card.appendChild(label);
        card.appendChild(text);
        const options = document.createElement('div');
        options.className = 'options';
        if (question.type.toLowerCase().includes('mcq') && parsedMcq.options.length > 0) {
          parsedMcq.options.forEach((opt) => {
    ...
    function parseMcqQuestion(sourceText) {
      const text = (sourceText || '').replace(/\s+/g, ' ').trim();
      const marker = text.indexOf(' A) ');
      if (marker === -1) {
        return { prompt: text, options: [] };
      }
      const prompt = text.substring(0, marker).trim();
      const optionsPart = text.substring(marker + 1);
      const options = [];
    ...
    function updateState(state) {
      ...
      const progress = Math.min(100, Math.round((answered / expectedQuestions) * 100));
