...
import { RobotAvatar, type AvatarVariant } from "./RobotAvatar";
...
type Avatar = { id: string; name: string; role: string; color: string; style: "boy" | "girl" | "male" | "robot"; variant?: AvatarVariant; };
type CoachTone = {
  onboardingPrompt: string;
  introTail: string;
  hintFallback: string;
  reviewFallback: string;
  yourTurnVoice: string;
  yourTurnText: string;
  readyLine: string;
  winLine: string;
  tryLabel: string;
...
const COACH_TONES: Record<string, CoachTone> = {
  raj: {
    onboardingPrompt: "First tell {name} what you know. Then {name} will choose the right pace and start your mission.",
    introTail: "We will go step by step and keep the method clear.",
    hintFallback: "Use the method {name} just showed you.",
    reviewFallback: "If you get stuck, {name} will reopen the pattern with one smaller step.",
    yourTurnVoice: "Your turn now: answer by voice or text.",
    yourTurnText: "Your turn now: type your answer and click Check.",
    readyLine: "Ready for the next step.",
    winLine: "Good work. Let us keep going.",
    tryLabel: "Try It",
    checkLabel: "Check",
    speakLabel: "Speak",
    skipLabel: "Skip",
  ...
  nova: {
    onboardingPrompt: "Tell {name} what you already know. Then {name} will make the mission feel easy to enter.",
    introTail: "We will keep it friendly, clear, and one move at a time.",
    hintFallback: "Try the same move {name} just modeled for you.",
    reviewFallback: "If this feels tricky, {name} will slow it down and reopen the pattern.",
    yourTurnVoice: "Your turn now: say it or type it. You are ready.",
    yourTurnText: "Your turn now: type your answer and tap Check.",
    readyLine: "Ready when you are.",
    winLine: "Nice one. You have this.",
    tryLabel: "Give It a Go",
    checkLabel: "Check",
    speakLabel: "Say It",
    skipLabel: "Pass",
    showStepsLabel: "Show Me"
  },
  priya: {
    onboardingPrompt: "Tell {name} your level fast. Then {name} will set the pace and get the mission moving.",
    introTail: "We will keep the pace sharp and the moves clean.",
    hintFallback: "Use the exact move {name} just showed you and hit it quickly.",
    reviewFallback: "If you miss it, {name} will break it down fast and put you back in.",
    yourTurnVoice: "Your turn now: answer by voice or text and lock it in.",
    yourTurnText: "Your turn now: type your answer and hit Check.",
    readyLine: "Set for the next one.",
    winLine: "Strong answer. Next one.",
    tryLabel: "Take the Shot",
    checkLabel: "Lock It In",
    speakLabel: "Speak Up",
    skipLabel: "Skip Ahead",
    showStepsLabel: "Break It Down"
  }
};

function toneForAvatar(avatarId?: string): CoachTone {
  return COACH_TONES[(avatarId || "").toLowerCase()] || COACH_TONES.raj;
}

function withCoachName(template: string, avatarName: string): string {
  return template.split("{name}").join(avatarName);
}
...
const AVATARS: Avatar[] = [
  { id: "raj",   name: "Raj",   role: "Learning Coach", color: "#E91E8C", style: "robot", variant: "screen" },
  { id: "nova",  name: "Nova",  role: "Learning Coach", color: "#E91E8C", style: "robot", variant: "round" },
  { id: "priya", name: "Priya", role: "Learning Coach", color: "#E91E8C", style: "robot", variant: "classic" }
];
...
const AVATAR_PORTRAIT_FOCUS: Record<string, { position: string; scale: number }> = {
  arya: { position: "center 10%", scale: 1.44 },
  ved: { position: "center 14%", scale: 1.38 },
  tara: { position: "center 12%", scale: 1.42 }
};
...
function AvatarFace({
  ...
  if (avatar.style === "robot") {
    return (
      <div style={{ transformOrigin: "50% 50%", animation: animated || speaking ? "avatarFloat 2.4s ease-in-out infinite" : "none" }}>
        <RobotAvatar
          size={size}
          speaking={speaking}
          compact
          variant={avatar.variant ?? "screen"}
          expression={speaking ? "encouraging" : "neutral"}
          accentColor="#E91E8C"
          baseColor="#3B3A8C"
        />
      </div>
  ...
  const src = AVATAR_STAGE_ART[avatar.id] || "/avatars/5.svg";
  const portraitFocus = AVATAR_PORTRAIT_FOCUS[avatar.id] || AVATAR_PORTRAIT_FOCUS.arya;
  const portraitScale = speaking ? portraitFocus.scale + 0.04 : portraitFocus.scale;
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(180deg, ${avatar.color}22 0%, rgba(255,255,255,0.98) 100%)`,
        boxShadow: "0 8px 20px rgba(15,23,42,0.08)",
        transformOrigin: "50% 50%",
        animation: animated || speaking ? "avatarFloat 2.4s ease-in-out infinite" : "none"
      }}
    >
      ...
      <img
        ...
        style={{
          ...
          height: "100%",
          objectFit: "cover",
          objectPosition: portraitFocus.position,
...
function AvatarCharacter({
  ...
  const src = AVATAR_STAGE_ART[avatar.id] || AVATAR_STAGE_ART.arya;
  return (
    <div
      className={`teacher-stage-avatar${speaking ? " speaking" : ""}`}
      style={{ ["--teacher-accent" as any]: avatar.color }}
      aria-label={`${avatar.name} teaching avatar`}
    >
...
function SpeakingTeacher({
  ...
  return <AvatarCharacter avatar={avatar} speaking={speaking || feedback != null} />;
}
...
function TutorContent() {
  ...
  const activeCoachTone = useMemo(() => toneForAvatar(activeAvatar.id), [activeAvatar.id]);
  ...
  const missionHintPrompt =
    ...
    question?.hint ||
    withCoachName(activeCoachTone.hintFallback, activeAvatar.name);
  ...
  const reviewLoopPrompt =
    ...
    withCoachName(activeCoachTone.reviewFallback, activeAvatar.name);
  ...
  const hasAnswerReadyQuestion = !!question && canAttemptAnswer && !sessionProgress.livesDepleted && !isTeachingBoard && !isSpeaking && !isEvaluatingAnswer && !isListening;
  ...
  }, [awaitingStudentResponse, hasAnswerReadyQuestion, check?.correct, isSpeaking, isTeachingBoard, sessionProgress.livesDepleted]);
  ...
  const stageStatusText = useMemo(() => {
    ...
    if (awaitingStudentResponse || hasAnswerReadyQuestion) {
      return micPermission === "denied"
        ? activeCoachTone.yourTurnText
        : activeCoachTone.yourTurnVoice;
    }
    return activeCoachTone.readyLine;
  }, [isTeachingBoard, isEvaluatingAnswer, isSpeaking, isListening, awaitingStudentResponse, hasAnswerReadyQuestion, micPermission]);
  const stageSceneMode = isTeachingBoard || isSpeaking ? "coach" : awaitingStudentResponse || isListening || isEvaluatingAnswer || !!check || hasAnswerReadyQuestion ? "student" : "coach";
  ...
  useEffect(() => {
    ...
    async function loadCatalog() {
      try {
        ...
        if (typeof window !== "undefined" && !isFreshLaunch) {
          try {
  ...
  }, [defaultRequestedExerciseGroup, isFreshLaunch, requestedChapterFromQuery, requestedCourseId, requestedExerciseGroupFromQuery, resumeStorageKey]);

  useEffect(() => {
    if (typeof window === "undefined" || !isFreshLaunch) return;
    try {
  ...
  }, [isFreshLaunch, resumeStorageKey]);
  ...
  }, [isFreshLaunch, requestedCourseId, resumeStorageKey]);
  useEffect(() => {
  ...
  async function startSession(options?: { preserveClassClock?: boolean; chapterCode?: string; exerciseGroup?: string }): Promise<boolean> {
    ...
    try {
      ...
      const coachIntro = data.lesson.duolingoLessonArc?.onboarding?.coachIntro || `${activeAvatar.name} will guide you through ${data.lesson.title}. ${activeCoachTone.introTail}`;
    ...
    async function runKickoff() {
      try {
        if (pendingKickoff === "welcome") {
          ...
          const coachIntro = lessonDuolingoArc?.onboarding?.coachIntro || `${activeAvatar.name} checks your comfort level before starting ${lessonTitle || previewChapter?.title || "this chapter"}. ${activeCoachTone.introTail}`;
      ...
      <section className="panel tutor-setup-panel">
        <div className={`tutor-quickstart${minimalDuolingoLayout ? " tutor-quickstart-duo" : ""}`}>
          ...
          <h2 className="tutor-qs-title">
            {minimalDuolingoLayout ? `Let ${activeAvatar.name} Set Up Your Learning Path` : `Meet ${activeAvatar.name}, Your AI Coach`}
          </h2>
          ...
          <p className="tutor-qs-tagline">
            {minimalDuolingoLayout
              ? (lessonDuolingoArc?.onboarding?.coachIntro || withCoachName(activeCoachTone.onboardingPrompt, activeAvatar.name))
              : "I will teach on the board, then push you through focused practice."}
          </p>

          
          <div className="tutor-onboard-group tutor-coach-picker">
            <div className="tutor-coach-picker-head">
              <p className="tutor-onboard-label">Choose your coach</p>
              <p className="tutor-coach-picker-note">Each coach keeps the same lesson flow, with a different avatar and Sarvam voice.</p>
            </div>
              ...
              {AVATARS.map((avatar) => {
                ...
                return (
                  <button
                    key={avatar.id}
                    type="button"
                    className={`tutor-avatar-card${isActive ? " active" : ""}`}
                  ...
                  >
                    <div className="tutor-avatar-card-img">
                      {avatar.style === "robot" ? (
                        ...
                        <AvatarFace avatar={avatar} size={52} animated={isActive} speaking={false} />
                      )}
                    </div>
                    <span className="tutor-avatar-card-name">{avatar.name}</span>
                    {isActive ? <span className="tutor-avatar-card-check">&#10003;</span> : null}
                  </button>
                );
              })}
            </div>
...
{minimalDuolingoLayout ? (
          ...
          <header className="vedic-topbar">
            <div className="vedic-topbar-row">
              <div className="vedic-topbar-main">
                <div className="vedic-topbar-headline">
                  <div>
                  ...
                  <div className="vedic-topbar-coach" style={{ ["--coach-accent" as string]: activeAvatar.color }}>
                    <AvatarFace avatar={activeAvatar} size={36} animated={isSpeaking} speaking={isSpeaking} />
                    <div className="vedic-topbar-coach-copy">
                      <span className="vedic-topbar-coach-kicker">Your Coach</span>
            ...
            <section className={`vedic-focus-card ${awaitingStudentResponse ? "spotlight" : ""}`}>
              ...
              <div className={`vedic-focus-stage ${stageSceneMode}`}>
                <div className="vedic-focus-scene">
                  <div className="vedic-focus-coach" style={{ ["--coach-accent" as string]: activeAvatar.color }}>
                    ...
                    <div className="vedic-focus-copy">
                      <div className="vedic-focus-coach-head">
                        <div>
                          <p className="vedic-focus-coach-label">{activeAvatar.name} | {activeAvatar.role}</p>
                          <h3>{stageSceneMode === "coach" ? missionPrompt : missionTryPrompt}</h3>
                        </div>
                        <span className={`vedic-focus-coach-chip ${stageSceneMode}`}>{stageSceneMode === "coach" ? "Explaining" : "Listening"}</span>
                      </div>
                      <p>{stageStatusText}</p>
                    </div>
                  </div>
                  ...
                  <div className="vedic-focus-content">
                        ...
                        <div className="vedic-focus-actions">
                          {showWorkedBoardSupport && !showInlineBoard ? (
                            <button className="button secondary" type="button" onClick={() => void teachOnBoard()} disabled={isTeachingBoard || isSpeaking}>
                              {activeCoachTone.showStepsLabel}
                            </button>
                          ) : null}
                          <button
                          ...
                          >{activeCoachTone.tryLabel}</button>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="vedic-answer-block vedic-answer-block-inline">
                          ...
                          <div className="vedic-action-row">
                            <button className="button vedic-primary-btn" onClick={() => void checkAnswer()} disabled={isEvaluatingAnswer || !canAttemptAnswer}>
                              {isEvaluatingAnswer ? "Checking..." : activeCoachTone.checkLabel}
                            </button>
                            <button
                              className="button secondary"
                              type="button"
                            ...
                            >
                              {micPermission === "denied" ? "Mic Blocked" : isListening ? "Listening..." : activeCoachTone.speakLabel}
                            </button>
                            {isListening ? (
                              <button
                                className="button secondary"
                            ...
                            <button className="button secondary" onClick={() => { void nextQuestion({ directToStudent: true, source: "skip" }); }}>{activeCoachTone.skipLabel}</button>
                          </div>
                        </div>
                      </>
                    )}
                    ...
                    {check ? (
                      <div className={`udemy-feedback ${check.correct ? "correct" : "wrong"}`}>
                        ...
                        <p className="muted">{check.correct ? (activeDuolingoStep?.instantFeedbackWin || missionCelebration || activeCoachTone.winLine) : (activeDuolingoStep?.instantFeedbackRetry || check.encouragement)}</p>
      ...
      <style>{`
        ...
        .tutor-choice-chip:hover {
          transform: translateY(-1px);
          box-shadow: 0 10px 24px rgba(15, 23, 42, 0.08);
        }
        .tutor-choice-chip:focus-visible {
        ...
        .tutor-coach-picker {
          ...
          background: linear-gradient(180deg, rgba(254, 252, 232, 0.95) 0%, rgba(236, 252, 203, 0.82) 100%);
        }
        .tutor-coach-picker-head {
          display: grid;
          gap: 0.25rem;
          margin-bottom: 0.7rem;
        }
        .tutor-coach-picker-note {
          margin: 0;
          color: #4b5563;
          font-size: 0.82rem;
          line-height: 1.4;
        ...
        .tutor-onboard-cta-row {
          display: flex;
          flex-wrap: wrap;
          gap: 0.65rem;
          align-items: stretch;
        }
        .tutor-onboard-primary-btn {
          flex: 1 1 240px;
          min-width: 220px;
        }
        .tutor-onboard-resume-btn {
          min-width: 180px;
          min-height: 48px;
          border-radius: 18px;
        }
        .tutor-onboard-cta-note,
        .tutor-onboard-cta-hint {
          margin: 0;
        }
        .tutor-qs-actions-hidden {
          display: none;
        }
        .tutor-qs-actions {
          margin-top: 1rem;
        ...
        .vedic-topbar-coach-copy {
          display: grid;
          gap: 0.05rem;
          text-align: left;
        }
        .vedic-topbar-coach-kicker {
          color: #64748b;
          font-size: 0.66rem;
          font-weight: 800;
          letter-spacing: 0.08em;
        ...
        .vedic-topbar-coach-copy strong {
          color: #0f172a;
          font-size: 0.9rem;
          line-height: 1;
        }
        ...
        .vedic-focus-coach-head {
          display: flex;
          align-items: flex-start;
          justify-content: space-between;
          gap: 0.55rem;
        }
        .vedic-focus-coach-label {
          margin: 0 0 0.2rem;
          color: #166534;
          font-size: 0.72rem;
          font-weight: 800;
        ...
        .vedic-focus-coach-chip.coach {
          border-color: #86efac;
          color: #166534;
          background: #f0fdf4;
        }
        .vedic-focus-coach-chip.student {
          border-color: #cbd5e1;
          color: #475569;
          background: white;
        }
