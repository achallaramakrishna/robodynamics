...
function TutorContent() {
  ...
  const missionReadPrompt =
    ...
    `${question?.questionText || "Listen to the question."} ${question?.hint || ""}`.trim();
  const missionTryPrompt =
    activeDuolingoStep?.tryPrompt ||
    activeDuolingoStep?.masteryCheck ||
  ...
  async function teachOnBoard() {
    ...
    await Promise.all([
      ...
      speak(activeDuolingoStep?.readAloudPrompt || `${toChatStylePrompt(question)} ${fallbackTeachingStep?.teacherLine || question.hint}.`)
    ]);
    if (runId !== teachRunRef.current) return;
    ...
    await speak(
      `${activeDuolingoStep?.tryPrompt || "Your turn now."} ${
        activeDuolingoStep?.masteryCheck || fallbackTeachingStep?.checkpointPrompt || "What should we do first?"
      } ${activeDuolingoStep?.reviewPrompt || fallbackTeachingStep?.microPractice || ""}`
    );
    if (runId !== teachRunRef.current) return;
    setAwaitingStudentResponse(true);
  }
  ...
  async function checkAnswer(answerOverride?: string, source?: "typed" | "voice") {
    ...
    try {
      ...
      if (data.correct) {
        ...
        if (autoTeachEnabled) {
          await speakRef.current(activeDuolingoStep?.instantFeedbackWin || "Great work. Moving to the next question.");
          await nextQuestion();
          return;
        }
        await speakRef.current(activeDuolingoStep?.instantFeedbackWin || "Great work. Tell me how you got that answer, then click Next Question.");
      } else {
        if (data.coachTip) {
          await speakRef.current(data.coachTip);
        }
        const retryLine = activeDuolingoStep?.instantFeedbackRetry || `Good attempt. ${activeTeachingStep?.checkpointPrompt || "Let us retry this with one smaller step."}`;
        if (autoTeachEnabled) {
          clearBoard();
        ...
        <div className="vedic-mission-app">
          <header className="vedic-topbar">
            <div className="vedic-topbar-row">
              <div className="vedic-topbar-main">
                <p className="vedic-topbar-label">{courseLabel} Mission</p>
                <h2 className="vedic-topbar-title">{missionTitle}</h2>
              </div>
          ...
          <div className="vedic-focus-shell">
            <section className={`vedic-focus-card ${awaitingStudentResponse ? "spotlight" : ""}`}>
                  ...
                  {lessonPath.map((item) => (
                    ...
                    >
                      <span className="vedic-path-pill-badge">
                        {item.status === "completed" ? "OK" : item.status === "locked" ? "L" : item.exerciseGroup}
                      </span>
                      <span className="vedic-path-pill-text">{item.subtopic}</span>
                    </div>
                  ))}
                </div>
              ) : null}
                ...
                <div className="vedic-focus-scene">
                  ...
                  <div className="vedic-focus-content">
                    {stageSceneMode === "coach" ? (
                      <>
                        <div className="vedic-focus-panel question">
                          <span className="vedic-prompt-label">Question</span>
                      ...
                      <>
                        ...
                        <div className="vedic-answer-block vedic-answer-block-inline">
                          ...
                          />
                          <p className="muted udemy-hint">{missionHintPrompt}</p>
                          {micPermission === "denied" ? (
              ...
              {showWorkedBoardSupport || showHelpDrawer ? (
                <div className="vedic-inline-tools vedic-inline-tools-compact">
                  {showWorkedBoardSupport ? (
                    <details className="vedic-inline-fold">
                      <summary>{showBoardPanel ? "Worked Board" : "Board Help"}</summary>
                      <div className="vedic-fold-body">
                        <div className="vedic-board-frame compact">
                          {showBoardPanel ? (
                            <AnimatedBoard
                              steps={boardSteps}
                              runId={boardRunId}
                            ...
                            <div className="ca-board-idle">
                              <span className="ca-idle-icon">{awaitingStudentResponse ? "Try" : "Plan"}</span>
                              <p className="ca-idle-text">
                                {awaitingStudentResponse ? "Open the board if you want to see the steps." : `${activeAvatar.name} can still walk through the method.`}
                              </p>
                            </div>
                          )}
                        </div>
