...
function simplifyQuestionText(raw: string): string {
  const text = String(raw || "").trim();
  if (!text) return "";
  return text.replace(/^Exercise\s+[A-I0-9]+\s*\([^)]*\):\s*/i, "").trim();
}
...
function TutorContent() {
  ...
  const activeQuestionText = question ? simplifyQuestionText(question.questionText || "") : "";
  ...
  const lessonListenLine = useMemo(() => {
    ...
    return `${activeQuestionText || question.questionText}. ${missionHintPrompt}`;
  }, [activeQuestionText, missionHintPrompt, missionReadPrompt, question, stageSceneMode]);
  ...
  async function speak(text: string): Promise<void> {
    ...
    if (allowBrowserVoiceFallback && "speechSynthesis" in window) {
  ...
  async function speakWithGuard(text: string, maxWaitMs = 9000): Promise<void> {
    ...
    let timeoutId = 0;
    const estimatedMs = 3000 + Math.max(0, text.trim().split(/\s+/).filter(Boolean).length * 420);
    await Promise.race([
      speak(text),
      new Promise<void>((resolve) => {
        timeoutId = window.setTimeout(() => {
          stopVoicePlayback();
          resolve();
        }, Math.max(1000, maxWaitMs, estimatedMs));
      })
    ]);
    if (timeoutId) {
  ...
  async function teachOnBoard() {
    ...
    if (screenplay.length) {
      ...
      for (const beat of screenplay) {
        ...
        if (runId !== teachRunRef.current) return;
        const cueIndex = TEACHING_CUE_ORDER.indexOf((beat.cue || "explain") as typeof TEACHING_CUE_ORDER[number]);
        const shouldOpenStudentTurn =
          beat.pauseType === "student_response" &&
          cueIndex >= TEACHING_CUE_ORDER.indexOf("demo");
        if (shouldOpenStudentTurn) {
          ...
          await speakWithGuard(activeQuestionText || activeDuolingoStep?.readAloudPrompt || beat.checkpointPrompt || activeTeachingStep?.checkpointPrompt || "Listen to the question.");
          if (runId !== teachRunRef.current) return;
        ...
        if (beat.pauseType === "student_response") {
          void logTutorEvent("SCREENPLAY_CHECKPOINT_DEFERRED", {
            mode: screenplayMode,
            beatId: beat.beatId,
            checkpointPrompt: beat.checkpointPrompt,
      ...
      setIsTeachingBoard(false);
      setAwaitingStudentResponse(true);
      await speakWithGuard(activeQuestionText || activeDuolingoStep?.readAloudPrompt || activeTeachingStep?.checkpointPrompt || "Listen to the question.");
      if (runId !== teachRunRef.current) return;
      await speakWithGuard(activeDuolingoStep?.tryPrompt || "Your turn now.", 5000);
      if (runId !== teachRunRef.current) return;
      return;
    ...
    await Promise.all([
      ...
      speak(activeQuestionText || activeDuolingoStep?.readAloudPrompt || `${toChatStylePrompt(question)} ${fallbackTeachingStep?.teacherLine || question.hint}.`)
    ]);
    if (runId !== teachRunRef.current) return;
    ...
    setAwaitingStudentResponse(true);
    await speakWithGuard(activeQuestionText || activeDuolingoStep?.readAloudPrompt || "Listen to the question.");
    if (runId !== teachRunRef.current) return;
    await speakWithGuard(activeDuolingoStep?.tryPrompt || "Your turn now.", 5000);
    if (runId !== teachRunRef.current) return;
  }
                    ...
                    {stageSceneMode === "coach" ? (
                      ...
                      <>
                        <div className="vedic-focus-panel question student">
                          ...
                          <p className="udemy-question-text"><strong>{activeQuestionText || question.questionText}</strong></p>
                          {question.visual?.svg ? (
          ...
          <div className="ca-body">
            ...
            <div className="ca-content">
              ...
              <div className="ca-board-bar">
                ...
                <button className="button secondary" type="button" onClick={() => void speak(`${activeQuestionText || question.questionText}. ${question.hint}`)}>
                  🔊 Speak
                </button>
                <button className="button secondary" type="button" onClick={clearBoard}>
                  Clear
