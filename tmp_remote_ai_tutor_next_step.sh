...
tar -xzf /tmp/ai_tutor_next_step.tgz -C /tmp
...
rm -rf .next
echo "NEXT_CLEANED"
npm run build >/tmp/rd_ai_tutor_web_build_next_step.log 2>&1 || (tail -n 120 /tmp/rd_ai_tutor_web_build_next_step.log; exit 1)

echo "BUILD_OK"
systemctl restart rd-ai-tutor-web
sleep 3
echo "WEB=$(systemctl is-active rd-ai-tutor-web)"
grep -n 'const targetExerciseGroup = getNextExerciseGroup' /opt/robodynamics/ai-tutor/web/app/ai-tutor/tutor/TutorClient.tsx || true
