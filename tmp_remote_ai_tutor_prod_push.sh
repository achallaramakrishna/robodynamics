...
tar -xzf /tmp/ai_tutor_prod_push.tgz -C /tmp
...
rm -rf .next
echo "NEXT_CLEANED"
npm run build >/tmp/rd_ai_tutor_web_build_prod_push.log 2>&1 || (tail -n 120 /tmp/rd_ai_tutor_web_build_prod_push.log; exit 1)
