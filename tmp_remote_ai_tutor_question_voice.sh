...
tar -xzf /tmp/ai_tutor_question_voice.tgz -C /tmp
...
rm -rf .next
echo "NEXT_CLEANED"
npm run build >/tmp/rd_ai_tutor_web_build_question_voice.log 2>&1 || (tail -n 120 /tmp/rd_ai_tutor_web_build_question_voice.log; exit 1)

echo "BUILD_OK"
systemctl restart rd-ai-tutor-web
sleep 3
echo "WEB=$(systemctl is-active rd-ai-tutor-web)"
grep -n 'await speak(question.questionText' /opt/robodynamics/ai-tutor/web/app/ai-tutor/tutor/TutorClient.tsx | head -n 10 || true
