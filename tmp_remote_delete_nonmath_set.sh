...
CREATE TEMPORARY TABLE tmp_bad_set AS
...
WHERE cs.course_id=34
  ...
  AND ep.exam_paper_id >= 38;

SELECT 'TO_DELETE', COUNT(*) FROM tmp_bad_set;
SELECT 'DEL_IDS', GROUP_CONCAT(exam_paper_id ORDER BY exam_paper_id) FROM tmp_bad_set;

DELETE FROM rd_exam_ai_summary
WHERE submission_id IN (
  SELECT submission_id FROM rd_exam_submission
  WHERE exam_paper_id IN (SELECT exam_paper_id FROM tmp_bad_set)
);
...
WHERE course_session_detail_id IN (SELECT course_session_detail_id FROM tmp_bad_set);
