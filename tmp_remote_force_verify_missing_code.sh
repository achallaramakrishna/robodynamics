...
echo "STEP1: switch catalog code temporarily"
mysql -uroot -p'Jatni@752050' -D robodynamics_db -e "UPDATE rd_ci_career_catalog SET career_code='$VERIFY_CODE', updated_at=NOW() WHERE ci_career_catalog_id=$ROW_ID;"

# ensure target roadmap code is empty before trigger
mysql -uroot -p'Jatni@752050' -D robodynamics_db -e "DELETE FROM rd_ci_career_roadmap WHERE module_code='APTIPATH' AND assessment_version='v3' AND career_code='$VERIFY_CODE';"
...
echo "STEP2: check generated rows"
...
echo "STEP4: revert catalog code"
mysql -uroot -p'Jatni@752050' -D robodynamics_db -e "UPDATE rd_ci_career_catalog SET career_code='AP3_CAR_0077', updated_at=NOW() WHERE ci_career_catalog_id=$ROW_ID;"
