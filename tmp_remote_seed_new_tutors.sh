...
SQL
mysql -uroot -pJatni@752050 -Nse \"USE robodynamics_db; SELECT course_id, course_name, course_category_id, category FROM rd_courses WHERE LOWER(course_name) IN ('aptitude and reasoning','financial literacy') ORDER BY course_id;\"
mysql -uroot -pJatni@752050 -Nse \"USE robodynamics_db; SELECT o.course_id, o.course_offering_id, c.course_name FROM rd_course_offerings o JOIN rd_courses c ON c.course_id=o.course_id WHERE LOWER(c.course_name) IN ('aptitude and reasoning','financial literacy') AND o.is_active=1 ORDER BY o.course_id, o.course_offering_id;\"
