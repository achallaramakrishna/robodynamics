...
& $pscp -batch -pw $pw "C:\roboworkspace\robodynamics\ai-tutor\tutor-api\app\main.py" "$user@${hostName}:/tmp/tutor_main.py"
if ($LASTEXITCODE -ne 0) { throw "Upload failed: main.py" }
...
& $pscp -batch -pw $pw "C:\roboworkspace\robodynamics\tmp_remote_ai_tutor_l1_repeat_fix_deploy.sh" "$user@${hostName}:/tmp/remote_ai_tutor_l1_repeat_fix_deploy.sh"
